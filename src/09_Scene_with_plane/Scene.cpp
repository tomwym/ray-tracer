...
int main() {
    ...
    Camera camera{300,150,nums::pi/3.f,transform};
    {
        ...
        mat.specular = 0.f;
        std::unique_ptr<Geometry> plane{std::make_unique<Plane>(transform, mat)};
        world.AddGeometry(std::move(plane));
    }
    {
        // middle sphere
    ...
    std::ofstream file{"res/scene_with_plane.ppm", std::ios::trunc};
    file << canvas.ToPPM();
}